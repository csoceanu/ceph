...
Cephadm includes automated container health checks for all Ceph daemons.
These checks run periodically to detect and report container issues.

The following configuration options control these checks:

- ``CONTAINER_HEALTH_CHECK_ENABLED``: Enable or disable health checks (default: True).
- ``CONTAINER_HEALTH_CHECK_INTERVAL``: Seconds between health checks (default: 30).
- ``CONTAINER_HEALTH_CHECK_TIMEOUT``: Seconds before a health check times out (default: 10).
- ``CONTAINER_HEALTH_CHECK_RETRIES``: Number of failed checks before marking a container as unhealthy (default: 3).
- ``CONTAINER_RESTART_ON_FAILURE``: Automatically restart unhealthy containers (default: True).
- ``CONTAINER_RESTART_MAX_ATTEMPTS``: Maximum restart attempts within the restart window (default: 5).
- ``CONTAINER_RESTART_WINDOW``: The time window in seconds for restart attempts (default: 300).
